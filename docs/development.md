...
The browser starts at **http://127.0.0.1:5173/**. This is an isolated local scenario. Add **?mode=server** to exercise the local Fastify API at port 4310. Stop development with Ctrl+C. The simulation compiler, API watcher, and browser dev server are started together. The browser uses compiled simulation exports; saving engine code triggers a rebuild.
